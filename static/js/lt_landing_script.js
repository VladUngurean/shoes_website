$(document).ready(function () {
  ...
  $(".countryselect").change(function () {
    ...
    $(".countryselect option[value=" + changeCountry + "]").prop(
      "selected",
      true
    );
...
function upd_int() {
  ...
  $("input[name=productsum]")
    .val(producInfo.productsum + producInfo.currency)
    .trigger("change");
  $(".productsum").html(producInfo.productsum + producInfo.currency);

  $("input[name=delivery]")
    .val(producInfo.delivery + producInfo.currency)
    .trigger("change");
  $(".delivery").html(producInfo.delivery + producInfo.currency);

  $("input[name=totalsum]")
    .val(producInfo.totalsum + producInfo.currency)
    .trigger("change");
